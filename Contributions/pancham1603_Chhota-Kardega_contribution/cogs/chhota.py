...
async def log_url(bot, embed):
    with open("log_channel.json", 'r') as file:
        data = json.load(file)
...
" cooked URL", description=f'https://chhotakarde.ga/{short}', color=discord.Color.blurple())
                        ...
                        embed = discord.Embed(title=f"{ctx.author.name}#{ctx.author.discriminator} created a URL", description=f'Short: https://chhotakarde.ga/{short}\nOriginal: {link}', color=discord.Color.blurple())
                        await log_url(self.bot, embed)
                        break
